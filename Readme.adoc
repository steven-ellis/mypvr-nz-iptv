...
- OpenMedia's TRAC tickets
 - #1093 External Recorder
 - #1419 DVB-I analysis
...
Freeview NZ is planning on link:https://www.advanced-television.com/2026/05/18/freeview-nextgen-launching-in-new-zealand/[adopting DVB-I late 2026]
which potentially provides a general way to get the DASH streams needed for IPTV recording.

There is an initial conversion script capable of turning a dvbi service list XML file into an XMLTV compatible format

[source,bash]
----
python3 dvbi_to_m3u8.py \
  <Service List XML URL or Filename>
  output.m3u8 \
  --lcn-offset 200
----
